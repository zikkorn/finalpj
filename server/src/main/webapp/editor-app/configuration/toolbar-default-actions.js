...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        validate: function(services){
        	services.$http({    method: 'POST',
                data: {
        			jsonXml : JSON.stringify(services.$scope.editor.getJSON())
        		},
                ignoreErrors: true,
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    if(json != null && json.properties != null && json.properties.usertaskassignment != null && json.properties.usertaskassignment != ''){
    	if(typeof(json.properties.usertaskassignment) == 'string'){
    		json.properties.usertaskassignment = JSON.parse(json.properties.usertaskassignment);
    	}
    	if(json.properties.usertaskassignment.assignment != null){
    		if(json.properties.usertaskassignment.assignment.candidateUsers != null){
    			if(!isArray(json.properties.usertaskassignment.assignment.candidateUsers)){
    				json.properties.usertaskassignment.assignment.candidateUsers = JSON.parse(json.properties.usertaskassignment.assignment.candidateUsers);
    			}
    		}
    	}
    }
    ...
    $scope.save = function (successCallback) {
        ...
        json.properties.process_id = $scope.processId;
        json.properties.name = $scope.processName;
        json = JSON.stringify(json);
