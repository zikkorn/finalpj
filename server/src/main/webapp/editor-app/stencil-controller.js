...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            var modelName,modelKey;
            $http({method: 'GET', url: KISBPM.URL.getModelInfo()}).success(function(data, status, headers, config){
            	modelName = data.name;
            	modelKey = data.key;
            })
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    if(selectedItem.properties[0].key == 'oryx-process_id'){
                    	selectedItem.properties[0].hidden = true;
                    	selectedItem.properties[1].hidden = true;
                    	$scope.processId = modelKey;
                    	$scope.processName = modelName;
                    }
